...
install_dependencies() {
    ...
    case $OS in
        ...
        "rhel")
            ...
            # Install Gradle manually for RHEL
            install_gradle_manual
            ;;
        "arch")
...
# Install Gradle manually for systems that don't have it in repos
install_gradle_manual() {
    if ! command -v gradle &> /dev/null; then
        log_info "Installing Gradle manually..."
        cd /tmp
...
build_katalyzer() {
    ...
    if [[ -d "katalyze" ]]; then
        ...
        # Build with Gradle
        if command -v gradle &> /dev/null; then
            gradle build -x test || log_warning "Katalyzer build failed, but continuing..."
        else
            log_warning "Gradle not found, skipping Katalyzer build"
        fi
