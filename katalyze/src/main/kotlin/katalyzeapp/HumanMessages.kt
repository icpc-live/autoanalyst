    ...
    scope.produce(capacity = UNLIMITED) {
        ...
        while (true) {
            ...
            newEntries.forEach { entry ->
                if (entry[Entries.contestTime] >= 0) {
                    while (contestInfo?.status is ContestStatus.BEFORE) {
                        kotlinx.coroutines.delay(10.milliseconds)
                        contestInfo = contestStateTracker.state?.info
                    }
                }
                val commentary = commentaryFromDBRow(entry, contestInfo!!)
                send(commentary)
            }
