...
class RejectedSubmissions(private val normalRankThreashold: Int) : RuleInterface {
    override fun run(contestFlow: Flow<ContestStateWithScoreboard>): Flow<Commentary> = flow {
        contestFlow.filterICPCJudgementEvents().filter { !(it.state.lastEvent as RunUpdate).newInfo.isAccepted() }
            .collect collect@{ contestStateWithScoreboard ->
                val runUpdate = contestStateWithScoreboard.state.lastEvent as RunUpdate
                val previousSubmissions =
                    contestStateWithScoreboard.state.runsBeforeEvent.values.filter { it.teamId == runUpdate.newInfo.teamId && it.time < runUpdate.newInfo.time }
                        .sortedBy { it.time }.toImmutableList()
                if (previousSubmissions.any { it.isAccepted() }) return@collect
                val rank = contestStateWithScoreboard.rankingAfter.getTeamRank(runUpdate.newInfo.teamId)
                val importance = if (rank <= normalRankThreashold) EventImportance.Normal else EventImportance.Whatever

                emit(Commentary.fromRunUpdateState(contestStateWithScoreboard.state, importance) { teamRef, problemRef ->
                    buildString {
