...
if (isset($_GET["team_id"]) && $_GET["team_id"] != "") {
    $query = sprintf("id in (%s)", $_GET["team_id"]);
} else {
...
$result = mysql_query("select * from teams where $query", $db);
...
for ($i = 0; $i < count($name); ++$i) {
    ...
    $school_name = $name[$i];
    $cntry = $country[$i];
    printf("<li><a href='team_feed.php?team_id=$tid'>$school_name</a> ($cntry)\n");
}
