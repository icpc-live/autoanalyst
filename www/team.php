...
<div id="video_container">
    ...
    <?php
        ...
        while ($result && ($row = mysqli_fetch_assoc($result))) {
            $counter++;
            if ($counter > 1) { print ", "; }
            printf("<a href='vlc://192.168.1.207/video/reaction/%d'>%d</a>", $row['submission_id'], $row['contest_time']);
        }
    ?>
</div>
        ...
        <div id="contestants">
            <table border>
                ...
                <?php
                  ...
                  $rows = mysqli_query($db, "select p.full_name as full_name, tp.role as role from persons p INNER JOIN team_persons tp ON p.id = tp.person_id where tp.team_id = $team_id order by tp.role, p.id");
		    if ($rows) {
